...
int Board::evaluateBoard(bool maximizingPlayer, int phase, int move) {
    int CurrentPlayer;
    bool flag;  //used to break out of nested for loop
    int counter = 0;    //used to count for eval8
    int eval1 = 0;
    int eval2 = 0;
    int eval3 = 0;
    int eval4 = 0;
        ...
        if (gamePositions[i].getPlayerOccupy() == CurrentPlayer) {	//if there is a 1, check if possible mills
            ...
            for (int j = 0; j < possibleMills.size(); j++) {
                ...
                for (int k = 0; k < 2; k++) {		//2 ints for each mill
                    if(possiblePositions.at(k) == CurrentPlayer){
    ...
    if (phase == 1) {
        return 18 * (eval1) + 26 * (2) + 1 * (3) + 9 * (eval4) + 10 * (5) + 7 * (6);
    }
    else if (phase == 2) {
        return 14 * (eval1) + 43 * (2) + 10 * (3) + 11 * (eval4) + 8 * (7) + 1086 * (eval8);
    }
    else {
        return 16 * (eval1) + 10 * (5) + 1 * (6) + 1190 * (eval8);
    }
}
...
int Board::getPosLastPlaced(){
    return positionLastPlaced;
}

void Board::setPositionLastPlaced(int position){
    positionLastPlaced = position;
}
