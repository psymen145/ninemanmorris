...
void NineManMorris::playAIRemove() {
	...
	for (int i = 0; i < 24; i++) {
		if (gameboard.getBoard(i) == 1) {
			...
			score = gameboard.evaluateBoard(2, phase);
			if (score > bestScoreToRemove) {
				bestScoreToRemove = score;
				posToRemove = i;	//store the position that was best
			}
			gameboard.setBoard(i, 1);
		}
	}

	//when we examine all possible positions to remove, then we can make a final decision and remove the piece
	gameboard.setBoard(posToRemove, 0);
}
