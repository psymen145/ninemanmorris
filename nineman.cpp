...
bool NineManMorris::isGameOver(){
    ...
    if(this->getNumTokensP1() < 3){
        cout << "player 2 has won" << endl;
        return true;
    }
    else if(this->getNumTokensP2() < 3){
...
void NineManMorris::play(int player, int token, int MoveTo){
    ...
    //changes to phase 2
    if(numOfTokensInHandP1 == 0 && numOfTokensInHandP2 == 0){
        phase = 2;
    }
    //changes to phase 3
    if((Game.getTokensPlacedP1() == 3 || Game.getTokensPlacedP2() == 3) && phase != 1){
        phase = 3;
    }
...
bool NineManMorris::isValid(int player, int token, int position){
    ...
    //1. player must still have token
        ...
        if(player == 1){
            if(this->getNumTokensP1() == 0){
                return false;
            }
        }
        if(player == 2){
            if(this->getNumTokensP2() == 0){
                return false;
            }
        }
        return true;
    ...
    //2. the spot they want to go to must be adjacent
        ...
        if(player == 1){
            if(Game.getBoard(token) != 1){
                ...
                if(Game.adjacentPositions(token,position)){
                    return true;
                }
                else{
                    return false;
...
bool NineManMorris::checkTriple(int currentplayer, int spot){
    ...
    if(flag == 1){
        while(flag == 1){
            cout << currentplayer << " please remove an opponent's token: " << endl;
            cin >> desiredRemove;
